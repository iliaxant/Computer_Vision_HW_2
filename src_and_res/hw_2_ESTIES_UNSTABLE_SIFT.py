...
# # #       1 - Key-point detection using SIFT in all four images       # # #

sift = cv.xfeatures2d_SIFT.create(500)

# # Image 1 # #

img1 = cv.imread('ESTIES_UNSTABLE/1.jpg')  # All images in ESTIES_UNSTABLE have resolution: (3072, 4080)

cv.namedWindow('Esties_1', cv.WINDOW_NORMAL)
cv.imshow('Esties_1', img1)
cv.waitKey(0)
...
cv.imshow('Esties_1', k_img1)
cv.waitKey(0)

# # Image 2 # #

img2 = cv.imread('ESTIES_UNSTABLE/2.jpg')

cv.namedWindow('Esties_2', cv.WINDOW_NORMAL)
cv.imshow('Esties_2', img2)
cv.waitKey(0)
...
cv.imshow('Esties_2', k_img2)
cv.waitKey(0)

# # Image 3 # #

sift = cv.xfeatures2d_SIFT.create(3250)

img3 = cv.imread('ESTIES_UNSTABLE/3.jpg')

cv.namedWindow('Esties_3', cv.WINDOW_NORMAL)
cv.imshow('Esties_3', img3)
cv.waitKey(0)
...
cv.imshow('Esties_3', k_img3)
cv.waitKey(0)

# # Image 4 # #

img4 = cv.imread('ESTIES_UNSTABLE/4.jpg')

cv.namedWindow('Esties_4', cv.WINDOW_NORMAL)
cv.imshow('Esties_4', img4)
cv.waitKey(0)
...
# Calculate the Homography to connect the images appropriately
M, mask = cv.findHomography(img_pt2, img_pt1, cv.RANSAC)

img_1_2 = cv.warpPerspective(img2, M, (img1.shape[1]+380, img1.shape[0]+322))
...
img_3_4 = cv.warpPerspective(img4, M, (img3.shape[1]+370, img3.shape[0]+255))
...
kp_1_2 = sift.detect(img_1_2)
# Compute descriptors of key-points in img1
desc_1_2 = sift.compute(img_1_2, kp_1_2)
...
# Detect key-points in img_1_2
kp_3_4 = sift.detect(img_3_4)
# Compute descriptors of key-points in img1
desc_3_4 = sift.compute(img_3_4, kp_3_4)
...
img_all = cv.warpPerspective(img_3_4, M, (img_1_2.shape[1]+2750, img_1_2.shape[0]+975))  # 800
img_all[0: img_1_2.shape[0], 0: img_1_2.shape[1]] = img_1_2
img_all = img_all[0:img_all.shape[0]-15, :]

cv.imshow('All-Matched', img_all)
cv.waitKey(0)

cv.imwrite('ESTIES_UNSTABLE/Stitched_Sift.jpg', img_all)
